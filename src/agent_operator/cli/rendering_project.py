...
from .rendering_fleet import format_fleet_mix_counts, render_fleet_items_table
...
def render_project_dashboard(
    ...
    *,
    ...
    header_lines = [f"project={payload.get('project')}", f"profile_path={payload.get('profile_path')}"]
    if isinstance(resolved, dict):
        ...
        header_lines.append("max_iterations=" + str(resolved.get("max_iterations", "-")) + " involvement=" + str(resolved.get("involvement_level", "-")))
    ...
    if isinstance(resolved, dict):
        ...
        if isinstance(success_criteria, list) and success_criteria:
            resolved_lines.append("success_criteria=" + " | ".join(str(item) for item in success_criteria[:3]))
    ...
    return Group(
        Panel("\n".join(header_lines), title=f"Project Dashboard: {payload.get('project')}", border_style="cyan"),
        Columns(
            [
                Panel("\n".join(resolved_lines), title="Resolved Defaults", border_style="green"),
                Panel(render_project_policy_table(active_policies, shorten_live_text=shorten_live_text) if isinstance(active_policies, list) else "-", title=f"Active Policies ({len(active_policies)})" if isinstance(active_policies, list) else "Active Policies", border_style="yellow"),
            ]
        ),
        ...
        Columns(
            [
                Panel(render_fleet_items_table(recent, shorten_live_text=shorten_live_text) if isinstance(recent, list) else "-", title=f"Recent ({len(recent)})" if isinstance(recent, list) else "Recent", border_style="blue"),
