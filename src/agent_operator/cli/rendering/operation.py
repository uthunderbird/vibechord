...
def render_dashboard(
    ...
    *,
    ...
    header_status = (
        ...
        f"scheduler={payload.get('scheduler_state')} "
        f"run_mode={payload.get('run_mode')} "
        f"involvement={payload.get('involvement_level')}"
    )
    header_lines = [
        header_status,
    ...
    if isinstance(active_session, dict):
        session_line = (
            ...
            f"[{active_session.get('adapter_key')}] "
    ...
    if isinstance(policy_coverage, dict):
        coverage_line = (
            f"policy_coverage: {policy_coverage.get('status') or '-'} "
