...
- `implemented`: `application/drive/` and `application/event_sourcing/` are already explicit
  submodule families
- `implemented`: broader command/query/runtime application families now live under
  `application/commands/`, `application/queries/`, and `application/runtime/`
- `implemented`: migrated flat `application/operation_*.py` family modules have been retired, so
  the canonical subpackages are now the only implementation homes for those families
