# ADR 0189: No Separate ADR Record
...
- `implemented`: the repository does not contain a standalone `design/adr/0189-*.md` file before
  this bookkeeping record
- `verified`: the repository does not contain any references to `ADR 0189` in code, tests, docs,
  or design records
...
An audit request on 2026-04-15 asked for line-by-line verification of `ADR 0189` against current
repository code and tests.

Current repository truth at the start of that audit:

- there was no `design/adr/0189-*.md` file
- there were no repository references to `ADR 0189`
- there was no git-history evidence of a prior tracked path under `design/adr/*0189*`
...
Do not create a new normative architecture decision under ADR 0189.

`ADR 0189` is reserved as a bookkeeping record only:
...
rather than using `ADR 0189` as a behavioral reference.

## Repository Evidence

- ADR directory listing showed numbering ends at `0188` before this file:
  [design/adr](/Users/thunderbird/Projects/operator/design/adr)
- Repository text search for `ADR 0189` / `0189` across design, docs, source, and tests produced
  no relevant matches:
...
- Git history inspection for `design/adr/*0189*` and commit messages mentioning `ADR 0189`
...
- `rg --files design/adr | rg '0189|189'` -> no `0189` ADR file before this record
- `rg -n "ADR 0189|ADR-0189|adr 0189|0189" design docs src tests` -> no ADR-0189 repository
  references before this record
- `git log --all --name-status -- 'design/adr/*0189*'` -> no tracked ADR-0189 history
- `git log --all --grep='ADR 0189' --oneline` -> no commit history naming ADR 0189
- `git log --all --grep='0189' --oneline` -> no commit history referencing `0189`
- full repository verification for this slice is `uv run pytest`

## Consequences

- Future requests for `ADR 0189` should not assume a hidden authoritative document exists in this
  repository.
