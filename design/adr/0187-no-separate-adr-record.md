# ADR 0187: No Separate ADR Record
...
- `implemented`: the repository does not contain a standalone `design/adr/0187-*.md` file before
  this bookkeeping record
- `verified`: the repository does not contain any references to `ADR 0187` in code, tests, docs,
  or design records
...
An audit request on 2026-04-15 asked for line-by-line verification of `ADR 0187` against current
repository code and tests.

Current repository truth at the start of that audit:

- there was no `design/adr/0187-*.md` file
- there were no repository references to `ADR 0187`
- there was no git-history evidence of a prior tracked path under `design/adr/*0187*`
...
Do not create a new normative architecture decision under ADR 0187.

`ADR 0187` is reserved as a bookkeeping record only:
...
rather than using `ADR 0187` as a behavioral reference.

## Repository Evidence

- ADR directory listing showed numbering ends at `0186` before this file:
  [design/adr](/Users/thunderbird/Projects/operator/design/adr)
- Repository text search for `ADR 0187` / `0187` across design, docs, source, and tests produced
  no relevant matches:
...
- Git history inspection for `design/adr/*0187*` and commit messages mentioning `ADR 0187`
...
- `rg --files design/adr | sort | rg '0187|187'` -> no `0187` ADR file before this record
- `rg -n "ADR 0187|0187|187" design docs src tests` -> no ADR-0187 repository references before
  this record
- `git log --all --name-status -- 'design/adr/*0187*'` -> no tracked ADR-0187 history
- `git log --all --grep='ADR 0187' --oneline` -> no commit history naming ADR 0187
- `git log --all --grep='0187' --oneline` -> no commit history referencing `0187`
- full repository verification for this slice is `uv run pytest`

## Consequences

- Future requests for `ADR 0187` should not assume a hidden authoritative document exists in this
  repository.
