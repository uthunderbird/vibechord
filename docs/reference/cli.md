...
`converse` remains callable as a transitional interactive NL dialogue surface while ADR 0219
settles the long-term dialogue shape. `operator converse OP` loads one operation's context;
omitting `OP` loads fleet-level context for active operations, with optional `--project` filtering
and `--context brief|full` context assembly control. Read-only turns answer directly. Turns that
imply writes show a proposed structured command and require confirmation before the existing
command path executes it. Use `ask` for stable single-shot NL queries.
