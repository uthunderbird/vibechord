...
from agent_operator.smoke import run_mixed_agent_selection_smoke
...
pytestmark = pytest.mark.skipif(
    os.environ.get("OPERATOR_RUN_MIXED_AGENT_SMOKE") != "1"
    or shutil.which("npx") is None
    or not _has_codex_oauth(),
    reason="requires OPERATOR_RUN_MIXED_AGENT_SMOKE=1, npx, and Codex OAuth",
)


@pytest.mark.anyio
async def test_mixed_agent_selection_smoke() -> None:
    settings = OperatorSettings()
    outcome = await run_mixed_agent_selection_smoke()
