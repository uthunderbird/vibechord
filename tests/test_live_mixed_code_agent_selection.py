...
from agent_operator.smoke import run_mixed_code_agent_selection_smoke
...
pytestmark = pytest.mark.skipif(
    os.environ.get("OPERATOR_RUN_MIXED_CODE_AGENT_SMOKE") != "1"
    or shutil.which("claude") is None
    or not _has_codex_oauth(),
    reason=("requires OPERATOR_RUN_MIXED_CODE_AGENT_SMOKE=1, local Claude CLI, and Codex OAuth"),
)


@pytest.mark.anyio
async def test_mixed_code_agent_selection_smoke_prefers_codex() -> None:
    settings = OperatorSettings()
    outcome = await run_mixed_code_agent_selection_smoke()
